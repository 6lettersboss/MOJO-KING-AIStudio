...
function App() {
  ...
  const handleRequestAnalysis = useCallback(async () => {
    ...
    try {
      ...
      if (!apiResponse.ok) {
          const errorResult = await apiResponse.json().catch(() => ({ error: 'An unknown server error occurred.' }));
          throw new Error(errorResult.error || `Server responded with status ${apiResponse.status}`);
      }
      
      const result: MojoResult = await apiResponse.json();
